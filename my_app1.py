...
def _norm(s: str) -> str:
    ...
    s = " ".join(s.lower().split())  # lower + espaces compactés
    return s
...
if "df" not in st.session_state:
    try:
        ...
        # >>> ICI : colonne full-name normalisée pour la recherche
...
# Filtrage (multi-mots, ordre libre, insensible aux accents/majuscules)
base_df = st.session_state.df
if st.session_state.q:
    tokens = [_norm(t) for t in st.session_state.q.split() if t.strip()]
